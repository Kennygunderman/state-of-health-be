...
1. **First verify prod has the coach tables** (the newest manual migration,
   `prisma/manual-migrations/coach/`). If not applied yet, apply that SQL to
   prod by hand — the baseline assumes the full current schema exists.
2. Then, from your machine (SSH tunnel to prod Postgres):
...
3. Sanity check: `npx prisma migrate status` should report "Database schema
   is up to date".
