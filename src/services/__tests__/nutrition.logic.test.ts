/**
 * `parseLogEntryBody` is the branch point of `POST /api/meal/:mealId/entry`
 * (Agent Action Plan §0.3.1): one endpoint, two mutually exclusive body shapes,
 * and a 400 verdict for everything else.
 *
...
describe('parseLogEntryBody — a body that is not an object', () => {
    it.each([
        ['null', null],
        ['undefined', undefined],
        ['a string', '{"name":"eggs"}'],
...
describe('parseLogEntryBody — a body that names neither shape', () => {
...
describe('parseLogEntryBody — a body naming both a personal and a catalog food', () => {
...
describe('parseLogEntryBody — the legacy shape', () => {
...
describe('parseLogEntryBody — the catalog shape', () => {
