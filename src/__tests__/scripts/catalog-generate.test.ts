...
const seedExistingFood = async (options: {
    ...
    const food = await makeCatalogFood({
        ...
        category: CATEGORY,
        food_state: options.foodState ?? 'cooked',
...
describe('an exhausted budget pauses the run, and a resume continues against the ledger', () => {
    ...
    it('carries the report state of earlier attempts into the resumed run`s report', async () => {
        const { runId } = await pausedRun();
        const resumed = createHarness({ maxBatches: 2, budgetLimit: 6, respond: secondBatchPayload });
...
describe('one run per coverage-plan version', () => {
    ...
    it('lets a narrowed slice advance the canonical run, and an unrestricted one finish it', async () => {
        ...
        const rest = createHarness({ coveragePlan: plan, maxBatches: null, respond: secondBatchPayload });
...
describe('a batch must answer with exactly the number of foods it was asked for', () => {
    ...
    it('leaves a short batch incomplete, and writes none of it', async () => {
        ...
        // column for: the retry asks for the full target again while
        // `readCategoryNames` tells the model to avoid what was written, so the
        // slice would end up with `partial + target` rows and the category's
        // volume would drift above the coverage plan.
        expect(await prisma.catalog_foods.count()).toBe(0);
        expect(await prisma.catalog_validation_records.count()).toBe(0);
...
describe('a branded candidate is refused at parse time', () => {
    ...
    const brandedBatch = (): Harness =>
        createHarness({
            // A one-slot slice answered with two proposals, one of them
            // refused: the ONE storable food fills the slice, and the refusal
            // fills nothing — which is why this batch completes while the
            // all-refused batch below does not. (The harness deliberately
            // over-answers the schema's maxItems; a real vendor would not, and
            // the refusal path is what this describe is about.)
            respond: () =>
                batchPayload(
                    foodPayload({
...
/* -------------------------------------------------------------------------- *
 * The tag vocabulary gate (review finding SCRBLD-F20).
 *
...
describe('a candidate whose tag lists leave the catalog vocabulary is refused at parse time', () => {
    ...
    /** The offending candidate first, a clean one second: the batch must lose only the first. */
    const taggedBatch = (tags: Record<string, unknown>): Harness =>
        createHarness({ respond: () => batchPayload(taggedPayload(tags), foodPayload()) });
    ...
    it.each(unknownCodeCases)(
        ...
        async (_description, tags, observed) => {
            ...
            // a candidate that cannot be stored earns neither.
            expect((await storedTagRows()).map((row) => row.canonical_name)).toEqual([FOOD_NAME]);
            expect(harness.evidenceCalls.map((call) => call.expectedName)).toEqual([FOOD_NAME]);
        },
    );
