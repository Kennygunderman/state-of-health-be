...
| `usda-vendor-limits`, `usda-rate-limit-recap` | the vendor hourly cap, the import's default share and the headroom between them, the physical attempts per logical call, the two request shapes, the nutrient numbers, the basis the nutrient array the import reads is stated on, and the vendor status a retried attempt follows | `scripts/lib/rateLimiter.ts`, `src/services/usda.service.ts`, `src/services/catalog.logic.ts` (the basis amount) |
